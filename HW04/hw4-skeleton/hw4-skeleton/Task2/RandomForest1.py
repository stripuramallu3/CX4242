...
class RandomForest(object):
    class __DecisionTree(object):
        ...
        def learn(self, X, y, attributes):
			...
			self.tree = self.buildTree(X, y, attributes, 0)

        def classify(self, record):
			# TODO: return predicted label for a single record using self.tree
			tree = self.tree
			return self.traverse(tree, record)

        def buildTree(self, X, y, attributes, depth):
			...
			for val in np.unique(vals):
				t[index][val] = self.buildTree(self.split2(X, index, val), self.split1(X, y, index, val), attributes, depth + 1)
			return t
        ...
        def entropy(self, y):
			tmp = y.tolist()
			frequency = [float(tmp.count(i)) / len(tmp) for i in [0,1]]
			ret = 0.0
        ...
        def findIndex(self, x, y, attributes):
			...
			for i in attributes:
				...
				for j in s:
					...
					tmp += len(splitted)/float(len(y)) * self.entropy(splitted)
				gain = r - tmp
				if gain > maxgain:
					maxgain = gain
					targetAttr = i
